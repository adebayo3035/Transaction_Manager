...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateDiscountFields() {
        if (discountTypeSelect.value === "percentage") {
            ...
            flatRateDiv.style.display = "none";
            percentageInput.required = true;
            flatInput.required = false;
        } else if (discountTypeSelect.value === "flat") {
            flatRateDiv.style.display = "flex";
        ...
        if (e.target.classList.contains('action-btn')) {
            ...
            switch (action) {
                ...
                case 'delete':
                    ...
                    break;
                case 'reactivate':
                    reactivatePromo(promoId);
                    break;
                case 'confirm-date':
                    confirmDateUpdate(promoId);
                    break;
            }
        }
    });
    ...
    `;
            ...
            const isActive = promo_details.delete_id == 0;

            if (isActive) {
                // Show both Update and Delete buttons for active promos
                actionButtons += `
            <button class="action-btn update-promo" data-action="update" data-id="${promo_details.promo_id}">Update Promo</button>
            <button class="action-btn delete-promo" data-action="delete" data-id="${promo_details.promo_id}">Delete</button>
        `;
            } else {
                // Show only Reactivate button for deleted promos
                actionButtons += `
            <button class="action-btn reactivate-promo" data-action="reactivate" data-id="${promo_details.promo_id}">Reactivate</button>
        `;
            }
    ...
    <tr id="updateDateBtnRow" style="display: none;">
        <td colspan="2" style="text-align: center;">
            <button class="action-btn confirm-date-update" data-action="confirm-date" data-id="${promo_details.promo_id}">Confirm Date Update</button>
        </td>
    </tr>
    `;
    ...
    function reactivatePromo(promoId) {
        if (confirm('Are you sure you want to Reactivate this Promo Offer?')) {
            fetch('backend/reactivate_promo.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('orderModal').style.display = 'none';
                        fetchPromos(currentPage); // Refresh the promo list
                    } else {
                        console.error('Failed to Reactivate Promo Offer:', data.message);
    ...
    addPromoForm.addEventListener('submit', function (event) {
        ...
        if (discountType === 'percentage') {
            ...
            const flatValue = parseFloat(flatInput.value);
            if (flatValue <= 0) {
                event.preventDefault();
    ...
    function handleFormSubmission(form) {
        form.addEventListener('submit', function (event) {
            ...
            // Get the discount type and appropriate value
            const discountType = document.getElementById('discount_type').value;
            let discountValue;

            if (discountType === 'percentage') {
                discountValue = document.getElementById('discount').value;
                // Remove the flat rate value from form data if it exists
                formData.delete('flatdiscount_value');
            } else if (discountType === 'flat') {
                discountValue = document.getElementById('flat_discount').value;
                // Remove the percentage value from form data if it exists
                formData.delete('discount_value');
            }

            // Add the discount value with the correct field name
