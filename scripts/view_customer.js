document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateCustomerDetails(customer_details) {
        ...
        // Populate Group and Unit
        const groupSelect = document.getElementById('selectOption');
        const unitSelect = document.getElementById('selectedUnit');

        // Set the current group and unit values
        groupSelect.value = customer_details.group_id;
        unitSelect.value = customer_details.unit_id;

        // Call function to dynamically load the units for the selected group
        loadUnits(customer_details.group_id, customer_details.unit_id);

        // Add listener for when the group is changed to dynamically load units
        groupSelect.addEventListener('change', function () {
            const selectedGroupId = this.value;
            loadUnits(selectedGroupId);
        });
