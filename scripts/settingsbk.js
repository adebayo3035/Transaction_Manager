...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const staffCustomerAccountBtn = document.getElementById('restrict-account');
        ...
        .then(data => {
            if (data.success) {
                ...
                if (userRole === 'Admin') {
                    ...
                    linkList.style.justifyContent = "center";
                    linkList.style.justifyContent = "center";
                    ...
                    resetQuestionAnswerBtn.style.display =
                        ...
                        staffCustomerAccountBtn.style.display =
    ...
    staffCustomerAccountBtn.onclick = (e) =>{
        e.preventDefault();
        toggleModal("driverCustomerRestrictionModal", "flex")
    }
    ...
    confirmButton.onclick = async () => {
        ...
        try {
            const response = await fetch('backend/restrict_block_staff_account.php', {
                method: 'POST',
    ...
    confirmButton2.onclick = async () => {
        ...
        try {
            const response = await fetch('backend/unrestrict_unblock_staff.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    // function to fetch eligible customer and driver for restriction or block
    ...
    accountTypeSelect.addEventListener('change', function () {
        ...
        if (selectedType === 'driver' || selectedType === 'customers') {
            fetchCustomerDriverAccounts(accountNameSelect, selectedType);
        } else {
...
// Fetch list of customers or drivers eligible to be blocked or restricted
function fetchCustomerDriverAccounts(selectOption, accountType = 'customer') {
    fetch('backend/get_customer_driver.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    .then(data => {
        if (data.success && Array.isArray(data.data)) {
            data.data.forEach(account => {
                addAccountOption(selectOption, account);
            });
        } else {
            console.error('Failed to fetch eligible accounts:', data.message);
        }
    })
    .catch(error => console.error('Error fetching accounts:', error));
}

// Add individual account to the dropdown with status indicator
function addAccountOption(selectOption, account) {
    const option = document.createElement('option');
    ...
    option.textContent = `${account.firstname} ${account.lastname} (${statusText})`;
    selectOption.appendChild(option);
}
// selectDriverCustomer = document.getElementById('driverCustomerName');
// driverCustomerAccountType = document.getElementById('driverCustomer').value;
// fetchCustomerDriverAccounts(selectDriverCustomer, driverCustomerAccountType);
