document.addEventListener("DOMContentLoaded", () => {
  ...
  const utils = {
    maskDetails: (details) =>
      details.slice(0, 2) + " ** ** " + details.slice(-3),
  ...
  const api = {
    fetchCustomers: (page = 1) => {
      ...
      const fetchData = fetch(
        `backend/get_customers.php?${params.toString()}`
      ).then((res) => res.json());
      ...
      Promise.all([fetchData, minDelay])
        .then(([data]) => {
          if (data.success && data.customers.length > 0) {
            ...
            utils.showError(
              elements.tables.customers.container,
              "No Customer Details at the moment"
            );
          }
        })
        .catch(() => {
    ...
    fetchGroups: () => {
      return fetch("backend/fetch_groups.php").then((response) =>
        response.json()
      );
    },
  ...
  const ui = {
    updateCustomersTable: (customers) => {
      ...
      customers.forEach((customer) => {
        ...
        const restrictionStatus =
          customer.restriction === 1 ? "Restricted" : "Not Restricted";
        const accountStatus =
          customer.delete_status === "Yes" ? "Deactivated" : "Activated";
        const restrictionClass =
          customer.restriction === 1
            ? "restricted-badge"
            : "not-restricted-badge";
    ...
    <td>
        <button class="view-details-btn" data-customer-id="${
...
`;
    ...
    populateCustomerDetails: (customerDetails, userRole) => {
      ...
      body.innerHTML = `
                <tr>
                    ...
                    <td><input type="text" id="dateCreated" value="${
                      customerDetails.date_created
                    }" disabled></td>
                </tr>
                <tr>
                    <td>First Name</td>
                ...
                <tr>
                    <td>Last Name</td>
                    <td><input type="text" id="lastname" value="${
                ...
                <tr>
                    <td>Email</td>
                    <td>
                        <input type="email" id="email" value="${utils.maskDetails(
                ...
                <tr>
                    <td>Phone Number</td>
                    <td>
                        <input type="text" id="phoneNumber" value="${utils.maskDetails(
                          customerDetails.mobile_number
                        )}">
                        <input type="hidden" id="originalPhoneNumber" value="${
                          customerDetails.mobile_number
                        }">
                        <div class="masking">
                ...
                <tr>
                    <td>Customer ID</td>
                    <td><input type="text" id="licenseNumber" value="${
                        ...
                        ${
                          customerDetails.restriction === 1
                            ...
                            : `<select id="restriction">
                                <option value="0" ${
                                  customerDetails.restriction === 0
                                    ? "selected"
                                    : ""
                                }>Not Restricted</option>
                                <option value="1" ${
                                  customerDetails.restriction === 1
                                    ? "selected"
                                    : ""
    ...
    <td colspan="2" class="action-buttons">
        ${
          customerDetails.delete_status !== "Yes" &&
          customerDetails.restriction !== 1
  ...
  const customer = {
    ...
    updateCustomer: (customerId) => {
      ...
      const originalPhoneNumber = document.getElementById(
        "originalPhoneNumber"
      ).value;
      ...
      const customerData = {
        ...
        email:
          emailInput === utils.maskDetails(originalEmail)
            ? originalEmail
            : emailInput,
        phone_number:
          phoneNumberInput === utils.maskDetails(originalPhoneNumber)
            ? originalPhoneNumber
            : phoneNumberInput,
      ...
      elements.forms.addCustomer.addEventListener("submit", (event) => {
        ...
        if (confirm("Are you sure you want to add new Customer?")) {
          ...
          api
            ...
            .then((data) => {
              if (data.success) {
                elements.messages.addCustomer.textContent =
  ...
  const setupEventListeners = () => {
    ...
    elements.forms.groupSelect.addEventListener("change", () => {
      customer.loadUnits(
        elements.forms.groupSelect.value,
        null,
        "selectedUnit"
      );
    });
  ...
  document.getElementById("addNewDriverBtn").onclick = function () {
    modalToggler('addNewDriverModal');
  }
