document.addEventListener("DOMContentLoaded", () => {
  ...
  const resolveOrderModal = document.getElementById("resolveOrderModal");
  ...
  function fetchOrders(page = 1) {
    ...
    if (allowedStatuses.includes(deliveryStatusInput)) {
      ...
      console.warn(
        "Invalid delivery status selected on frontend:",
        deliveryStatusInput
      );
    ...
    const fetchData = fetch(
      `backend/fetch_order_summary.php?${params.toString()}`
    ).then((res) => res.json());
  ...
  async function fetchOrderDetails(orderId) {
    try {
      const response = await fetch("backend/fetch_order_details.php", {
        method: "POST",
      ...
      const orderStatus = order.delivery_status;
      const isReported = order.reported == 1;

      // Update modal header
      document.getElementById("orderID").textContent = orderId;

      // Ensure items is an array before passing
      ...
      if (orderStatus === "Assigned") {
        showButton(reassignButton);
        showButton(cancelOrderBtn);
      } else if (orderStatus === "In Transit" && !isReported) {
        showButton(cancelOrderBtn);
      } else if (orderStatus === "In Transit" && isReported) {
        showButton(resolveReportButton);
      }
  ...
  /**
   * Helper to show & enable a button
   */
  ...
  function populateOrderDetails(order, items) {
    ...
    fragment.appendChild(
      createRow("Delivery Status", order?.delivery_status || "N/A")
    );
    fragment.appendChild(
      createRow("Date Last Modified", order?.updated_at || "N/A")
    );
    ...
    fragment.appendChild(
      createRow("Delivery Fee", order?.delivery_fee || "N/A")
    );
    fragment.appendChild(
      createRow("Number of Packs", order?.pack_count || "N/A")
    );
    fragment.appendChild(
      createRow("Refunded Amount", order?.revenue?.refunded_amount || "0.00")
    );
    fragment.appendChild(
      createRow("Is Order Reported", order?.reported ? "Yes" : "No")
    );

    // Promotion details with safe access
    if (order?.promotion) {
      if (
        order.promotion.percentage_discount !== null &&
        order.promotion.percentage_discount !== undefined
      ) {
        fragment.appendChild(
          createRow(
            "Percentage Discount (%)",
            order.promotion.percentage_discount
          )
        );
      }
      if (
        order.promotion.discount_value !== null &&
        order.promotion.discount_value !== undefined
      ) {
        fragment.appendChild(
          createRow("Discount Value (N)", order.promotion.discount_value)
        );
      }
      if (
        order.promotion.promo_code !== null &&
        order.promotion.promo_code !== undefined
      ) {
        fragment.appendChild(
          createRow("Promo Code", order.promotion.promo_code)
        );
      }
    }

    fragment.appendChild(
      createRow("Balance", order?.revenue?.retained_amount || "0.00")
    );
    ...
    if (order?.assigned_admin) {
      fragment.appendChild(
        createRow(
          "Order Assigned To",
          `${order.assigned_admin.firstname || ""} ${
            order.assigned_admin.lastname || ""
          }`
        )
      );
    }
    ...
    if (order?.delivery_status === "Cancelled" && !order?.approver) {
      fragment.appendChild(
        createRow("Order Approved By", "Customer Cancelled Order")
      );
    ...
    if (order?.customer) {
      ...
      fragment.appendChild(
        createRow("Customer's Mobile Number", order.customer.phone || "N/A")
      );
      fragment.appendChild(
        createRow("Customer's Address", order.customer.address || "N/A")
      );
    }
  ...
  // Attach event listener to the Resolve Report button
  document
    .getElementById("resolve-report")
    ?.addEventListener("click", handleResolveReportClick);

  function handleResolveReportClick() {
    // Close all modals
    document.querySelectorAll(".modal").forEach((modal) => {
      modal.style.display = "none";
    });

    // Open the Resolve Order modal
    const resolveOrderModal = document.getElementById("resolveOrderModal");
    if (resolveOrderModal) {
      resolveOrderModal.style.display = "block";
    } else {
      console.error(
        "Resolve Order Modal (#resolveOrderModal) not found in the DOM."
      );
    }
  }
  ...
  resolveReportButton.addEventListener("click", (event) => {
    const orderId = event.currentTarget.dataset.orderId;
    console.log("Order ID:", orderId);
    handleResolveReportClick();
  });
  ...
  document
    .querySelector("#resolveOrderModal .close")
  ...
  // });
  document
    .getElementById("filterDeliveryStatus")
  ...
  window.addEventListener("click", (event) => {
    if (
      ...
      event.target === reassignForm ||
      event.target === packModal ||
      event.target === resolveOrderModal
    ) {
      orderModal.style.display = "none";
      reassignForm.style.display = "none";
      packModal.style.display = "none";
          ...
          packs: Object.keys(packsData)
            .map((packId) => {
              ...
              if (!pack || !pack.pack_info || !pack.items) {
                console.warn(
                  "Invalid pack structure for packId:",
                  packId,
                  pack
                );
                return null;
              ...
              return {
                ...
                created_at:
                  pack.pack_info.created_at || new Date().toISOString(),
                items: (pack.items || [])
                  .map((item) => ({
  ...
  const displayPackDetails = (order) => {
      ...
      packHeader.innerHTML = `
            <div class="pack-header-content">
                ...
                <div class="pack-info">
                    ...
                    <p class="pack-date">Created: ${formatDate(
                      pack.created_at
                    )}</p>
                </div>
            ...
            <table class="pack-items-table">
                    ...
                    ${pack.items
                      .map(
                        ...
                        <tr>
                            ...
                            <td class="item-total">${formatCurrency(
                              item.total_price
                            )}</td>
                        </tr>
                    `
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="pack-subtotal">${formatCurrency(
                          pack.total_cost
                        )}</td>
                    </tr>
                </tfoot>
    ...
    const totalAmount = order.packs.reduce(
    ...
    summary.innerHTML = `
        ...
        <div class="summary-content">
            ...
            <div class="summary-row">
                ...
                <span class="value">${order.packs.reduce(
