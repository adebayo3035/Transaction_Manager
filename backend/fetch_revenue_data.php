...
$totalRevenueQuery = "
    SELECT SUM(retained_amount) AS totalRevenue 
    FROM revenue 
...
$approvedOrdersQuery = "SELECT COUNT(*) as approvedOrders FROM revenue WHERE status = 'Completed'";
$approvedOrdersResult = $conn->query($approvedOrdersQuery);
$approvedOrders = $approvedOrdersResult->fetch_assoc()['approvedOrders'];

$declinedOrdersQuery = "SELECT COUNT(*) as declinedOrders FROM revenue WHERE status = 'Cancelled'";
