...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($result->num_rows > 0) {
            ...
            $delete_status = $admin_data['delete_status'];

            if ($block_id == 0 && $delete_status == NULL) {
                if ($encrypted_password === $admin_password) {
                    ...
                    //Update promo table for expired promos
                     // Step 5: Check and update expired promotions
                     $updatePromoQuery = "UPDATE promo SET status = 0 WHERE end_date < NOW() AND status = 1";
                     if ($conn->query($updatePromoQuery)) {
                         error_log("Expired promotions updated successfully.");
                     } else {
                         error_log("Failed to update expired promotions: " . $conn->error);
                     }
                ...
                echo json_encode(["success" => false, "message" => "Account is blocked or Deactivated. Kindly contact Super Admin."]);
                exit;
            }
        }
    } else {
