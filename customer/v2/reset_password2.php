...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmtCheckAttempts = $conn->prepare("SELECT reset_attempts, last_attempt_date FROM customer_password_reset_attempts WHERE email = ? AND DATE(last_attempt_date) = CURDATE()");
    ...
    if ($result->num_rows > 0) {
        ...
        if ($row['secret_answer'] !== $hashedSecretAnswer) {
            ...
            if ($result->num_rows == 0) {
                ...
                $stmtResetAttempts = $conn->prepare("UPDATE customer_password_reset_attempts SET reset_attempts = ?, last_attempt_date = NOW() WHERE email = ? AND DATE(last_attempt_date) = CURDATE()");
            ...
            $stmtResetAttempts = $conn->prepare("UPDATE customer_password_reset_attempts SET reset_attempts = 0, last_attempt_date = NOW() WHERE email = ? AND DATE(last_attempt_date) = CURDATE()");
