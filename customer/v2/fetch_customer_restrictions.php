...
logActivity("Fetching RESTRICT logs for Customer internal ID $internalId | Page $page | Limit $limit");

try {
    // Count total
    $countQuery = "SELECT COUNT(*) FROM account_restriction_audit_log WHERE account_type = 'CUSTOMERS' AND account_id = ? AND action_type = 'RESTRICT'";
    ...
    logActivity("Fetched " . count($records) . " RESTRICT records out of $totalRecords for customer ID $customerId (internal ID: $internalId)");
    ...
    logActivity("Error fetching RESTRICT logs for customer ID $customerId: " . $e->getMessage());
