document.addEventListener("DOMContentLoaded", () => {
  ...
  const api = {
    fetchOrders: (page = 1, status = "") => {
      ...
      const minDelay = new Promise((resolve) =>
        setTimeout(resolve, CONFIG.minSpinnerTime)
      );

      const url = `${CONFIG.apiEndpoints.fetchOrders}?page=${page}&limit=${CONFIG.itemsPerPage}&delivery_status=${status}`;
      ...
      Promise.all([fetchData, minDelay])
        .then(([data]) => {
          if (data.success && data.orders.length > 0) {
            ...
            utils.showError(
              elements.tables.orders.container,
  ...
  const ui = {
    ...
    updatePagination: (totalItems, currentPage, itemsPerPage) => {
      ...
      for (let i = start; i <= end; i++) {
        ...
        btn.addEventListener("click", () => {
          state.currentPage = i;
          api.fetchOrders(i, state.deliveryStatus);
        });
      ...
      const lastBtn = utils.createButton(
        "Last »",
        totalPages,
        currentPage === totalPages
      );
    ...
    displayOrderDetails: (order) => {
      ...
      const summaryRows = [
        ...
        {
          label: "Total Order",
          value: utils.formatCurrency(order.total_order || 0),
        },
        {
          label: "Service Fee",
          value: utils.formatCurrency(order.service_fee || 0),
        },
        {
          label: "Delivery Fee",
          value: utils.formatCurrency(order.delivery_fee || 0),
        },
        { label: "Discount", value: utils.formatCurrency(order.discount || 0) },
        {
          label: "Total Amount",
          value: utils.formatCurrency(order.total_amount || 0),
        },
      ...
      const showReceiptBtn = ["Delivered", "Cancelled", "Declined"].includes(
        order.delivery_status
      );
      const showRatingBtn =
        ["Delivered", "Cancelled on Delivery"].includes(
          order.delivery_status
        ) && !order.is_rated;
      const showAlreadyRated =
        ["Delivered", "Cancelled on Delivery"].includes(
          order.delivery_status
        ) && order.is_rated;

      elements.buttons.receipt.style.display = showReceiptBtn
        ? "block"
        : "none";
      elements.buttons.driverRating.style.display = showRatingBtn
        ? "block"
        : "none";
      elements.buttons.decline.style.display =
      ...
      if (elements.messages.alreadyRated) {
        elements.messages.alreadyRated.style.display = showAlreadyRated
          ? "flex"
          : "none";
      }
    ...
    `;
            ...
            <div class="pack-card-body">
                <div class="pack-info">
                    ...
                    <div class="info-item">
                        ...
                        <span class="info-value total-amount">${utils.formatCurrency(
                          pack.total_cost
                        )}</span>
                    </div>
                </div>
                      ...
                      .map(
                        ...
                        <div class="food-item">
                            ...
                            <div class="food-details">
                                ...
                                <span class="price">${utils.formatCurrency(
                                  item.price_per_unit
                                )}</span>
                                <span class="total">${utils.formatCurrency(
                                  item.total_price
                                )}</span>
                            </div>
                        </div>
      ...
      const totalItems = order.packs.reduce(
      ...
      summaryCard.innerHTML = `
        ...
        <div class="summary-content">
            ...
            <div class="summary-row grand-total">
                ...
                <div class="summary-value">${utils.formatCurrency(
                  totalAmount
                )}</div>
            </div>
        </div>
    ...
    printReceipt: () => {
      const orderDetails =
        document.querySelector("#orderDetailsTable").outerHTML;
      const now = new Date();
    ...
    loadPackDetails: (orderId) => {
      api
        ...
        .then((response) => {
          if (!response.success) {
            console.error(
              "Failed to fetch Order food pack details:",
              response.message
            );
            // ui.showError(response.message || 'Unknown error');
            utils.showError(
              elements.tables.orderDetails.container,
              response.message
            );
            return;
          }
        ...
        .catch((error) => {
          ...
          // ui.showError('Failed to load order details. Please try again.');
          utils.showError(
            elements.tables.packDetails.container,
  ...
  const setupEventListeners = () => {
    ...
    const filterSelect = document.getElementById("filterDeliveryStatus");
    filterSelect.addEventListener("change", () => {
      state.deliveryStatus = filterSelect.value;
      api.fetchOrders(1, state.deliveryStatus);
    });
  ...
  const init = () => {
    ...
    api.fetchOrders(state.currentPage, state.deliveryStatus);
  };
