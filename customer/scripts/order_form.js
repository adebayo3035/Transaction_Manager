const orderSummaryTable = document
  .getElementById("orderSummaryTable")
  .querySelector("tbody");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  addFoodButton.addEventListener("click", function () {
    ...
    const foodName =
      foodSelect.options[foodSelect.selectedIndex].getAttribute(
        "data-food_name"
      );
    const foodPrice = parseFloat(
      foodSelect.options[foodSelect.selectedIndex].getAttribute("data-price")
    );
    ...
    // ✅ Reset the food select and quantity input
    foodSelect.selectedIndex = 0; // reset the <select> back to default option
    quantityInput.value = ""; // clear the quantity box
  });
  ...
  submitOrderButton.addEventListener("click", function (event) {
    ...
    if (actualPackCount < packCount) {
      alert(
        `Warning: You created ${packCount} packs but only ${actualPackCount} contain items.`
      );
      return;
    }
    ...
    const deliveryFee = 0.1 * totalAmount;
...
const createPack = () => {
  ...
  if (Number.isNaN(qty) || qty < 1) {
    if (typeof showErrorModal === "function")
      showErrorModal("Enter a valid pack quantity (minimum 1).");
    else alert("Enter a valid pack quantity (minimum 1).");
    return;
  }

  const confirmed = window.confirm(
    `Create ${qty} pack${qty > 1 ? "s" : ""}? This will lock the pack count.`
  );
  if (!confirmed) return;
...
function addNewFoodItem(foodId, foodName, foodPrice, quantity, packId) {
  ...
  orderItems.sort((a, b) =>
    a.pack_id.localeCompare(b.pack_id, undefined, { sensitivity: "base" })
  );
