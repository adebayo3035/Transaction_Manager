...
logActivity("Fetching RESTRICT logs for Driver ID $driverId | Page $page | Limit $limit");

try {
    // Count total
    $countQuery = "SELECT COUNT(*) FROM account_restriction_audit_log WHERE account_type = 'DRIVER' AND account_id = ? AND action_type = 'RESTRICT'";
    ...
    logActivity("Fetched " . count($records) . " RESTRICT records out of $totalRecords for Driver ID $driverId");
    ...
    logActivity("Error fetching RESTRICT logs for Driver ID $driverId: " . $e->getMessage());
