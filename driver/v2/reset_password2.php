...
function checkResetAttempts($conn, $email) {
    $stmt = $conn->prepare("SELECT reset_attempts, last_attempt_date FROM driver_password_reset_attempts WHERE email = ? AND DATE(last_attempt_date) = CURDATE()");
...
function updateResetAttempts($conn, $email) {
    ...
    if ($result->num_rows == 0) {
        ...
        $stmtUpdate = $conn->prepare("UPDATE driver_password_reset_attempts SET reset_attempts = ?, last_attempt_date = NOW() WHERE email = ? AND DATE(last_attempt_date) = CURDATE()");
