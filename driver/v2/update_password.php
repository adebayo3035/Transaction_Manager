...
function updatePassword($conn, $email, $hashedPassword)
{
    ...
    if ($stmt->execute()) {
        ...
        $stmtReset = $conn->prepare("UPDATE driver_password_reset_attempts SET reset_attempts = 0, last_attempt_date = NOW() WHERE email = ? AND DATE(last_attempt_date) = CURDATE()");
        if (!$stmtReset) {
            $conn->rollback();
